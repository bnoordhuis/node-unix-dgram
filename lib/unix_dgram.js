...
var _unix_addr = binding._unix_addr;
...
Socket.prototype.bind = function(path) {
  var err = bind(this.fd, _unix_addr(path));
  if (err < 0)
...
Socket.prototype.connect = function(path) {
  var err = connect(this.fd, _unix_addr(path));
  if (err < 0) {
...
Socket.prototype.send = function(buf, offset, length, path, callback) {
  ...
  if (this.connected) {
    ...
    err = sendto(this.fd, buf, offset, length, _unix_addr(path));
  }
