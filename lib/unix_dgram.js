...
function recv(status, buf, path, fds) {
  var rinfo = {
    size: buf.length,
    address: {},
    path : path
...
Socket.prototype.send = function(buf, offset, length, path, fds, callback) {
  var err;
  ...
  if (this.connected) {
    ...
    err = send(this.fd, buf, fds || []);
  } else {
    if (typeof fds === 'function') {
      callback = fds;
      fds = [];
    } else if (typeof fds === 'undefined') {
      fds = [];
      callback = undefined;
    ...
    err = sendto(this.fd, buf, offset, length, path, fds || []);
  }
...
// compatibility
Socket.prototype.sendto = function(buf, offset, length, path, fds, callback) {
  return this.send(buf, offset, length, path, fds, callback);
};
